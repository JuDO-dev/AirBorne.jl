...
"""
function movingAverage(array::Vector; windowSize::Union{Int,Nothing}=nothing, startFrom::Int=1)
    out = Array{Union{Float64,Nothing}}(undef,length(array)) # Preallocate memory
    start(i) = windowSize === nothing ? 1 : max(i-windowSize,1)
    factor(i) = windowSize === nothing ? i+1-startFrom : min(windowSize,i+1-startFrom)
    sum_value = 0
    for i = startFrom:length(array)
       sum_value+= array[i]
