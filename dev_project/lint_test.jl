...
# Copy Configuration
cp(
    joinpath(@__DIR__, "..", ".JuliaFormatter.toml"),
    joinpath(@__DIR__, "temp", "lint", ".JuliaFormatter.toml");
    force=true,
)
...
@info(
    "Testing if files are identical, if not a message with bash commands will appear to preview the changes on each file."
)
...
for (root, dirs, files) in walkdir(src_dir)
    ...
    for file in files
        ...
        if (size(added(diff), 1) > 0) || (size(removed(diff), 1) > 0)
            ...
            @info(
                "diff --color '" *
                    joinpath(src_dir, relpath, file) *
                    "' '" *
                    joinpath(dst_dir, relpath, file) *
                    "'"
            )
        end
    end
